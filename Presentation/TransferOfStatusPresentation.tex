...
\title[$B^\pm\to(K^+K^-\pi^+\pi^-)_DK^\pm$]{CKM angle \texorpdfstring{$\gamma$}{gamma} determination in \texorpdfstring{$B^\pm\to DK^\pm$, $D\to K^+K^-\pi^+\pi^-$ decays}{B to DK, D to K+K-pi+pi-} decays}
\author{Martin Tat}
...
\begin{frame}{Binned fit of $D\to K^+K^-\pi^+\pi^-$}
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item{$\mathcal{A}(B^+\to DK^+) = \mathcal{A}(\bar{D^0}) + r_Be^{i(\delta_B + \gamma)}\mathcal{A}(D^0)$}
    \end{itemize}
    \item{Split into bins $-i, ..., -1, +1, ..., +i$ and integrate over phase space:}
    \begin{itemize}
...
\begin{frame}{Global mass fit}
  \begin{itemize}
    ...
    \item{Generated $1000$ toy datasets $\implies$ No biases in CP observables!}
  \end{itemize}
  \begin{figure}
    \centering
...
\begin{frame}{Double tag yields}
  \begin{itemize}
    \setlength\itemsep{1.3em}
    \item{Current $\SI{3}{\per\femto\barn}$ insufficient, expect $\SI{20}{\per\femto\barn}$ by end of 2023}
